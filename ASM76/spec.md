...
- Instruction sets
	- **76-Base** is capable of manipulating registers, memory and 8-, 32- and 64-bit integers.
	- **76-Float** is able to deal with floating point values.
	- **76-Vector** can do vector mathematics.
- Registers
...
#### **LCMM _size in bytes_** - set local memory size
Specifies the local memory size. It does not has a maximum limit in theory and is 16KB by default. As the command runs, the data in the memory will be cleared and initialized with all zero. Then it will copy previous data into the memory. If the previous data is longer, it will be truncated.

#### **LDxA _local memory address_, _register_** - load local long/int/byte from an address
→ LDLA LDIA LDBA
...
#### **LDxR _address register_, _register_** - load local long/int/byte from an address stored in a register to a register
→ LDLR LDIR LDBR
...
Stores 8/4/1 byte(s) from *register* to *local memory address*.
...
#### **ADDx/MINx/MTPx/DIVx/MODx _$A_, _$B_** - arithmetic operations
→ ADDL ADDI ADDB MINL MINI MINB MTPL MTPI MTPB DIVL DIVI DIVB MODL MODI MODB
...
#### **CMPx _$A_, _$B_** - compare two long/int/byte arithmetically
→ CMPL CMPI CMPB
...
→ ANDL ANDI ANDB OR_L OR_I OR_B XORL XORI XORB

#### **NOTx _$A_** - boolean not for long/int/byte
→ NOTL NOTI NOTB
