# =============================================================================
#  <GL/gltools.jl> Module Name: None
# -----------------------------------------------------------------------------
#  OpenGL simple toolkits
#  From: https://github.com/yurivish/Julia-Playground/blob/master/simple_gl.jl
#  LICENSE: NONE
# =============================================================================

function glGenOne(glGenFn)
    id::Ptr{GLuint} = pointer(GLuint[0])
    glGenFn(1, id)
...
function getInfoLog(obj::GLuint)
	...
	if maxlength > 0
		...
		sizei::Ptr{GLsizei} = pointer(GLsizei[0])
		getInfo(obj, maxlength, sizei, buffer)
		length = unsafe_load(sizei)
		unsafe_string(pointer(buffer), length)
	else
		""
	end
end
...
function validateShader(shader)
	success::Ptr{GLint} = pointer(GLint[0])
...
function createShaderProgram(f, vertexShader, fragmentShader)
	...
	glLinkProgram(prog)
	status::Ptr{GLint} = pointer(GLint[0])
...
function printGLInfo()
    println("GLSL version: ",    unsafe_string(glGetString(GL_SHADING_LANGUAGE_VERSION)))
    println("OpenGL version: ",  unsafe_string(glGetString(GL_VERSION)))
    println("OpenGL vendor: ",   unsafe_string(glGetString(GL_VENDOR)))
    println("OpenGL renderer: ", unsafe_string(glGetString(GL_RENDERER)))
end
