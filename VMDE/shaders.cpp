...
int Shaders::init_shaders(const GLchar* vsh_src_ptr, const GLchar* fsh_src_ptr) {
	...
	if (!success) {
		...
		rb_raise(rb_eRuntimeError, "VSH compilation failed: %s", infoLog);
	}
	...
	if (!success) {
		...
		rb_raise(rb_eRuntimeError, "FSH compilation failed: %s", infoLog);
	}
}
...
int Shaders::link_program() {
	...
	if (!success) {
		...
		rb_raise(rb_eRuntimeError, "shaders linking failed:\n%s", infoLog);
	}
